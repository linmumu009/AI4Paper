...
import type { IdeaBenchmark } from '../types/paper'
  ...
  <div class="h-full flex flex-col overflow-hidden">
    ...
    <div class="shrink-0 px-4 py-2 border-b border-border bg-bg">
      <div class="flex items-center justify-between gap-3">
        <h1 class="text-base font-semibold text-text-primary flex items-center gap-2">
          <span class="text-xl">📊</span> 评测回放
        </h1>
        <button
          class="text-xs px-3 py-1.5 rounded-full border border-border bg-transparent text-text-muted active:bg-bg-hover"
    ...
    <div class="flex-1 overflow-y-auto p-4">
      ...
      <div v-if="showForm" class="bg-bg-card border border-border rounded-lg p-4 mb-4 space-y-3">
        <input v-model="newName" type="text" placeholder="基准名称" class="w-full px-3 py-2 text-sm rounded-lg border border-border bg-bg-elevated text-text-primary placeholder:text-text-muted focus:outline-none focus:border-tinder-pink" />
        <textarea v-model="newDesc" rows="2" placeholder="描述（可选）" class="w-full px-3 py-2 text-sm rounded-lg border border-border bg-bg-elevated text-text-primary placeholder:text-text-muted focus:outline-none focus:border-tinder-pink resize-none" />
        <div class="flex justify-end gap-2">
          <button class="text-xs px-3 py-1.5 rounded-full border border-border bg-transparent text-text-muted active:bg-bg-hover" @click="showForm = false">取消</button>
          <button class="text-xs px-3 py-1.5 rounded-full bg-gradient-to-r from-[#fd267a] to-[#ff6036] text-white border-none active:opacity-90" @click="handleCreate">创建</button>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="p-4 active:bg-bg-hover" @click="expandedId = expandedId === bm.id ? null : bm.id">
